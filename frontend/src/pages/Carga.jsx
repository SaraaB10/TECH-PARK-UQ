...
// ─── Escenarios reales (solo los que tienen endpoint en el backend) ────────────
const ESCENARIOS = [
    {
        id: 'completo',
...
// Fases visuales mapeadas a operaciones reales
// fase 'pre'  → verificar estado del backend
// fase 'load' → llamar al endpoint de carga (la operación real)
// fase 'post' → consultar resumen post-carga para confirmar integridad
const PASOS_CARGA = [
...
// ─── Hook: backend ping + estado de carga real ────────────────────────────────
...
// ─── Hook: resumen real del sistema consultando el backend ────────────────────
function useSistemaReal() {
    const [sistema,   setSistema]   = useState(null)  // null = sin datos
    ...
    const cargarResumen = useCallback(async () => {
        ...
        try {
            ...
            // Consultar datos reales en paralelo
            const [zonasRes, atraccionesRes, operadoresRes, alertasRes] =
            ...
            setSistema({
                ...
                // visitantes no tiene endpoint GET lista, usamos suma de visitantes por zona
                visitantes:  Array.isArray(zonas)
                    ? zonas.reduce((acc, z) => acc + (z.visitantesActuales || 0), 0)
                    : 0,
                // senderos = nodos del mapa (se obtiene por separado si hace falta; usamos atracciones como proxy)
                ...
                <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
                    ...
                    <div className="flex flex-wrap gap-3">
                            ...
                            style={{
                                border: `0.5px solid ${
                                    online   ? 'rgba(34,197,94,0.25)'
                                        : checking ? 'rgba(233,196,106,0.25)'
                                            :            'rgba(230,57,70,0.25)'
                                }`,
                            }}
                        >
                            ...
                            <div>
                                <div className="text-sm font-bold"
                                     style={{ fontFamily: 'var(--font-display)', color: online ? '#22c55e' : checking ? '#e9c46a' : '#e63946' }}>
                        ...
                        <div className="glass rounded-xl px-5 py-3 flex items-center gap-3">
                            ...
                            <div>
                                <div className="text-sm font-bold"
                                     style={{ fontFamily: 'var(--font-display)', color: cargado ? '#22c55e' : 'var(--c-muted)' }}>
                        ...
                        <div className="glass rounded-xl px-5 py-3 flex items-center gap-3">
                            ...
                            <div>
                                <div className="text-xs font-mono font-bold"
                                     style={{ fontFamily: 'var(--font-mono)', color: '#457b9d' }}>
                                    localhost:8080
                                </div>
...
function PanelCarga({ onLog, onCargaCompleta }) {
    ...
    // Avanza un paso visualmente
    async function avanzarPaso(paso, totalPasos, completadosRef) {
        if (abortRef.current) throw new Error('Cancelado')
        setPasoActual(paso.id)
        onLog({ tipo: 'log', msg: `▸ ${paso.label}…` })
        completadosRef.count++
        setProgreso(Math.round((completadosRef.count / totalPasos) * 100))
    ...
    async function ejecutarCarga() {
        ...
        try {
            // ── FASE PRE: verificar backend ──────────────────────────────────
            const pasosPre = PASOS_CARGA.filter(p => p.fase === 'pre')
            for (const paso of pasosPre) {
                setPasoActual(paso.id)
                onLog({ tipo: 'log', msg: `▸ ${paso.label}…` })
                ...
                if (paso.id === 'ping') {
                    ...
                    const estadoRes = await parqueService.estadoCarga()
                    const msg = estadoRes.data?.mensaje || 'Estado verificado'
                    onLog({ tipo: 'ok', msg: `✓ ${msg}` })
                }
            ...
            // ── FASE LOAD: llamada real al backend ───────────────────────────
            const pasosLoad = PASOS_CARGA.filter(p => p.fase === 'load')
            for (const paso of pasosLoad) {
                setPasoActual(paso.id)
                onLog({ tipo: 'log', msg: `▸ ${paso.label}…` })
                count.count++

                if (paso.id === 'envio') {
                    // Esta es la llamada real — si falla, lanzamos el error
                    ...
                    // paso 'procesando' — solo visual, ya ejecutamos la carga
                    onLog({ tipo: 'ok', msg: `✓ ${paso.label}` })
                }
            ...
            // ── FASE POST: verificar datos cargados en el backend ────────────
            const pasosPost = PASOS_CARGA.filter(p => p.fase === 'post')

            // Consultas reales en paralelo
            const [zonasRes, atraccionesRes, operadoresRes, alertasRes] =
                await Promise.allSettled([
                    zonaService.getAll(),
                    atraccionService.getAll(),
            ...
            for (const paso of pasosPost) {
                setPasoActual(paso.id)
                onLog({ tipo: 'log', msg: `▸ ${paso.label}…` })
                count.count++

                // Loguear lo que encontramos
            ...
            <div className="p-6">
                        ...
                        >
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="text-sm font-bold"
                                      style={{ fontFamily: 'var(--font-display)', color: escenario === e.id ? e.color : 'var(--c-text)' }}>
                                    {e.nombre}
                                </span>
                                {escenario === e.id && (
                            ...
                            <div className="mt-2 text-xs font-mono"
                                 style={{ fontFamily: 'var(--font-mono)', color: e.color, opacity: 0.8 }}>
                ...
                {error && !cargando && (
                    <div className="flex items-start gap-3 rounded-xl p-4 mb-4"
                        ...
                        <div>
                            <p className="text-sm font-semibold" style={{ color: '#e63946', fontFamily: 'var(--font-display)' }}>
                                Error en la carga
                            </p>
                    ...
                    <div className="rounded-2xl p-5" style={{ background: 'rgba(255,255,255,0.02)', border: '0.5px solid var(--c-border)' }}>
                        <div className="flex items-center justify-between text-xs mb-2">
                            <span style={{ color: 'var(--c-muted)', fontFamily: 'var(--font-display)' }}>
                                {cargando
                                    ? (PASOS_CARGA.find(p => p.id === pasoActual)?.label || 'Procesando…')
                                    : 'Completado'}
                            </span>
                            <span className="font-mono font-bold"
                                  style={{ fontFamily: 'var(--font-mono)', color: esc.color }}>
                                {progreso}%
                            </span>
                        </div>
                                        ...
                                        {ok
                                            ...
                                            : activo
                                                ? <div className="w-3 h-3 rounded-full border-2 border-t-transparent animate-spin flex-shrink-0"
                                                       style={{ borderColor: esc.color, borderTopColor: 'transparent' }} />
                                                : <Circle size={12} style={{ color: 'var(--c-border)', flexShrink: 0 }} />
                                        }
                                        <span className="truncate"
                                              style={{ color: ok ? 'var(--c-dim)' : activo ? 'var(--c-text)' : 'rgba(107,114,128,0.5)' }}>
                                            {paso.label}
                                        </span>
                                    </div>
                                )
        ...
        <div className="glass rounded-2xl overflow-hidden">
            <div className="px-6 py-4 border-b flex items-center justify-between" style={{ borderColor: 'var(--c-border)' }}>
                <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center"
                         style={{
                             background: online ? 'rgba(34,197,94,0.12)' : 'rgba(230,57,70,0.12)',
                             border: `0.5px solid ${online ? 'rgba(34,197,94,0.25)' : 'rgba(230,57,70,0.25)'}`,
                         }}>
                ...
                >
                    <div className="flex items-center gap-3">
                        <div className="relative w-10 h-10 rounded-xl flex items-center justify-center"
                             style={{ background: online ? 'rgba(34,197,94,0.15)' : 'rgba(230,57,70,0.15)' }}>
                            {online
                                ? <Wifi    size={20} style={{ color: '#22c55e' }} />
                                : <WifiOff size={20} style={{ color: '#e63946' }} />
                            }
                            {online && (
                            ...
                            <>
                                <div className="text-xl font-bold font-mono"
                                     style={{
                                         ...
                                         color: latency < 100 ? '#22c55e' : latency < 300 ? '#f4a261' : '#e63946',
                                     }}>
                                    {latency}ms
                                </div>
                        ...
                        {ENDPOINTS.map((ep) => (
                            <div key={ep.path} className="flex items-center gap-3 rounded-lg px-3 py-2"
                                ...
                                <code className="text-xs font-mono flex-1 truncate"
                                      style={{ fontFamily: 'var(--font-mono)', color: 'var(--c-muted)', fontSize: 11 }}>
                                    {ep.path}
                                </code>
                                {ep.ok
                                    ? <CheckCircle size={12} style={{ color: '#22c55e',                    flexShrink: 0 }} />
...
// ─── Resumen del sistema — datos reales ───────────────────────────────────────
                ...
                <div>
                    ...
                    <p className="text-xs" style={{ color: 'var(--c-muted)' }}>
                        {cargandoResumen
                            ? 'Consultando backend…'
                            : cargado
                                ? 'Datos activos en memoria'
                                : 'Sin datos cargados — usa el panel de carga'}
                    </p>
                </div>
                {cargandoResumen
                    ? <div className="ml-auto w-4 h-4 rounded-full border-2 border-t-transparent animate-spin"
                           style={{ borderColor: '#6a4c93', borderTopColor: 'transparent' }} />
                    : cargado
            ...
            <div className="p-5 grid grid-cols-2 sm:grid-cols-3 gap-3">
                {items.map(({ label, value, icon: Ico, color }) => (
                    <div key={label} className="rounded-xl p-4 relative overflow-hidden group"
                        ...
                        <div className="absolute -top-4 -right-4 w-16 h-16 rounded-full opacity-0 group-hover:opacity-10 transition-opacity blur-xl"
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="text-2xl font-bold"
                                 style={{ fontFamily: 'var(--font-display)', color: cargado ? color : 'var(--c-border)' }}>
                                {cargado ? value : '0'}
                            </div>
...
// ─── Historial de cargas — construido en sesión con datos reales ──────────────
                ...
                <div>
                    ...
                    <p className="text-xs" style={{ color: 'var(--c-muted)' }}>
                        {historial.length === 0
                            ...
                            : `${historial.length} ejecución${historial.length !== 1 ? 'es' : ''} en esta sesión`}
                    </p>
                </div>
            </div>
                                ...
                                <td className="hidden lg:table-cell">
                                    <div className="flex items-center gap-2 text-xs" style={{ color: 'var(--c-muted)' }}>
                                        {h.estado === 'EXITOSO' && h.resumen ? (
                                            <>
                                                <span>{h.resumen.zonas} zonas</span>
                                                <span>·</span>
                                                <span>{h.resumen.atracciones} atracciones</span>
                                                <span>·</span>
                                                <span>{h.resumen.operadores} operadores</span>
                                            </>
                                        ) : <span>—</span>}
                                    </div>
                                </td>
    ...
    return (
        <div className="glass rounded-2xl overflow-hidden flex flex-col" style={{ height: 380 }}>
            ...
            <div className="px-5 py-3 border-b flex items-center justify-between flex-shrink-0"
                ...
                <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center"
                         style={{ background: 'rgba(13,15,20,0.8)', border: '0.5px solid var(--c-border)' }}>
            ...
            <div className="px-4 py-2 border-t flex items-center justify-between flex-shrink-0"
                ...
                <span className="text-xs flex items-center gap-1.5"
                      style={{ fontFamily: 'var(--font-mono)', color: '#22c55e', fontSize: 10 }}>
...
export default function Carga() {
    ...
    const { sistema, cargado, cargandoResumen, cargarResumen } = useSistemaReal()

    // Historial de sesión — empieza vacío, sin datos DEMO
    ...
    // Al montar, verificar si ya hay datos cargados en el backend
    useEffect(() => {
        window.scrollTo(0, 0)
        cargarResumen()
    }, [cargarResumen])
    ...
    // Recibe el escenario, la duración real y el resumen real del backend post-carga
    function handleCargaCompleta(esc, duracionMs, resumenReal) {
        // Refrescar el resumen del sistema con datos reales
        cargarResumen()

        // Agregar al historial de sesión con datos reales (no DEMO)
        setHistorial(prev => [{
            id:        Date.now(),
            ...
            resumen:   resumenReal, // { zonas, atracciones, operadores, visitantes, senderos, alertas }
        }, ...prev].slice(0, 8))
    }

    // KPIs calculados sobre historial real de la sesión
    const kpis = [
        {
            label:  'Cargas exitosas',
            value:  historial.filter(h => h.estado === 'EXITOSO').length,
            accent: '#22c55e',
            icon:   CheckCircle,
        },
        {
            label:  'Con errores',
            value:  historial.filter(h => h.estado === 'ERROR').length,
            accent: '#e63946',
            icon:   XCircle,
        },
        {
            label:  'Última carga',
            value:  historial[0]?.hora || '—',
            accent: '#2a9d8f',
            icon:   Clock,
        },
        {
            label:  'Latencia actual',
            value:  status === 'online' && latency ? `${latency}ms` : '—',
            accent: '#6a4c93',
            icon:   Signal,
        },
    ]
